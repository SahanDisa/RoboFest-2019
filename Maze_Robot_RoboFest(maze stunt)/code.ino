...
void loop() // put your main code here, to run repeatedly
{
  ...
  float fdist;
  float rdist;
  float ldist; // front, right, and left distances.
  float fduration;
  float rduration;
  float lduration; // front, right, and left travel time in echoPin.
  float fdur[5];
  float rdur[5];
  float ldur[5]; // Arrays which store the values of five durations... We will take only the median value(afer sorting) with error bearing capacity of 40%.
  float ldista[5];
      ...
      if ((rsensor == false) && (frontdist() >= 40)) // If both front and right are open..
      {
        ...
        else
        {
          ...
          /*Serial.print("for the jth turn ..");Serial.print(" =");Serial.print(j);
            Serial.print(" the i value is ");Serial.print(i);Serial.print("and the dir is ..");Serial.println(dir[i]);*/
          timefr = tfr + 65 * nf;
          nf = nf + 1;
          stopit(); delay(1000);
          for (int g = 1; g <= 10; g++) {
